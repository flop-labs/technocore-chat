...
Every tool is "build a URL, GET it once, hand back the body", so exactly one function
has to know how bytes are actually moved — and that is the one function whose answer
...
The seam is deliberately the *whole* transport and nothing else: status and body out,
no exception translation, no URL building. Everything a tool's answer depends on —
which query keys survive, what a 429 body becomes — stays in `server.py`, written once,
identical on both platforms. A seam drawn any higher would be two copies of the part
that matters.
"""
...
# One GET. Returns `(status, body)` for every HTTP answer, success or failure — a 429 is
# a value here, not an exception, because its body is the part the model needs. Raise
# `OSError` (and only `OSError`) when there was no HTTP answer at all.
Fetch = Callable[[str, dict[str, str], float], Awaitable[tuple[int, str]]]


def _blocking_get(url: str, headers: dict[str, str], timeout: float) -> tuple[int, str]:
...
async def urllib_fetch(url: str, headers: dict[str, str], timeout: float) -> tuple[int, str]:
    ...
    """
    return await anyio.to_thread.run_sync(_blocking_get, url, headers, timeout)
