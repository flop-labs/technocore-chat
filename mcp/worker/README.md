...
A rebuild during development produces the same name — `technocore_mcp-0.14.4-py3-none-any.whl`
...
`[tool.uv]` in `pyproject.toml`. The dependency is an exact `technocore-mcp==0.14.4`, so with
