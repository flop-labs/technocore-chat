...
def _write_record(
    ...
    nonce: int | None = None,
    sig: str | None = None,
    # `object`, not `int | None`: callers pass the value unvalidated (a GET query string, a
    # raw JSON field) precisely so the one bounds-check below owns rejecting every bad shape.
    re: object = None,
) -> tuple[dict, bool]:
