...
def _reap_reason(
    ...
    `st` is forwarded to `_reapable` unchanged -- the one stat the reap loop is allowed to
    reuse, and only for the pre-lock call (see `_reapable`). Meaningless when `guard_room`
    is not None: a guard note's own mtime never decides its reap, so there is nothing here
    for a stat of the note itself to save.
    """
    if guard_room is not None:
        return "orphaned" if _guard_orphaned(guard_room, now) else None
    return _reapable(path, now, stillborn_rule, st)
    ...
    for sub, suffix, stillborn_rule in (("rooms", ".jsonl", True), ("notes", ".txt", False)):
        ...
        for entry in _walk(root / sub, suffix):
            try:
                ...
                # reusing the stat just taken. See _reap_reason.
                guard_room = _guard_note_room(root, base, entry)
                if not _reap_reason(entry.path, guard_room, now, stillborn_rule, st):
                    continue
