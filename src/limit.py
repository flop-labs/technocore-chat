...
# The longest a client-supplied identity may be before it is refused. Both bounds above cap
# the NUMBER of entries, not the size of a key, and the key comes from a header whose value
# the caller chooses: app.MAX_HEADER_BYTES (8192) is measured across the whole header block,
# so one header can carry very nearly 8 KiB on its own. 20_000 buckets plus 50_000
# identities at that size is roughly 530 MiB of long-lived strings against a 128 MiB
# container -- a resource exhaustion that escapes the documented caps rather than one of the
# caps, which is why it is fixed here and not documented as a limit.
#
# 64 characters, because no real spelling of a peer comes close: an IPv4 address is at most
# 15, an IPv6 address 45, 46 with a zone id of any sane length, and a bracketed
# host:port form 53. The bound is on LENGTH ONLY, deliberately -- not on IP syntax. An
# operator may legitimately point CHAT_CLIENT_IP_HEADER at something that is not
# IP-shaped at all (a `Forwarded: for=...` element, a CDN's opaque client token), and
# rejecting those on shape would silently collapse every caller into a single shared
# bucket: the exact global-lockout failure PROXY_IP_HEADERS exists to make visible.
...
def client_ip(request: Request, ip_header: str = "") -> str:
    ...
    A trusted header's value is length-bounded before it is returned. Everything downstream
    keeps this string as a long-lived dictionary key -- `_buckets`, `_identities`, the
    waiter slots -- and those bound how MANY keys exist, not how big one is, so an
    8 KiB-per-request identity turns an entry cap into a memory amplifier. Over-long values
    fall back to the socket peer rather than being truncated: a truncated prefix is still
    attacker-chosen and still plentiful, whereas the peer is fail-closed -- callers sending a
    malformed header share the proxy's single bucket, which is stricter than a fresh one and
    never looser.
    """
    if ip_header:
        forwarded = request.headers.get(ip_header, "").split(",")[0].strip()
        if forwarded and len(forwarded) <= MAX_IDENTITY_CHARS:
            return forwarded
