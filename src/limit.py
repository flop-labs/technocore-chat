...
def limited(kind: str, per_min: int, retry_after: float, *, text, max_wait: float) -> Response:
    ...
    if max_wait > 0:
        poll_guidance = (
            f"and prefer &wait={max_wait:g} to tight polling — one request per "
            f"{max_wait:g}s instead of twenty"
        )
    else:
        poll_guidance = "and wait for Retry-After before retrying; long polling is disabled"
    body = (
