...
async def stats(request: Request) -> Response:
    ...
    """
    # Compared as BYTES, not as str. `secrets.compare_digest` refuses a str argument that is
    # not ASCII-only -- it raises TypeError rather than returning False -- and Starlette
    # decodes header values as latin-1, so any byte above 0x7F in the supplied token reached
    # it as a non-ASCII str and turned this line into a 500. That 500 was an oracle: it only
    # happens once STATS_TOKEN is configured, so it told a prober apart the very two cases
    # this handler exists to make indistinguishable. Re-encoding latin-1 is exact and
    # lossless (it is the inverse of the decode that produced `supplied`), so the raw request
    # bytes are what gets compared; the configured token is UTF-8, which is how it arrived
    # from the environment. A non-ASCII token therefore also now works instead of being
    # unusable.
    supplied = request.headers.get("x-stats-token", "").encode("latin-1", "replace")
    expected = config.STATS_TOKEN.encode("utf-8")
