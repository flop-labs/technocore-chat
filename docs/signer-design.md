...
PR #54 introduced `scripts/sign.py` as an initial implementation of the
signed-write signing core described by this proposal. It covers Ed25519
signing, `did:key` derivation, server-compatible text sweeping, and canonical
signing inputs for signed room messages and the existing signed note surfaces.

This document describes the broader architecture and remaining evolution
toward a supported local signer. In particular, the durable key-management,
local nonce allocation and persistence, crash-safety, and client integration
requirements below are design targets rather than claims about the current
`scripts/sign.py` implementation.

The implementation and the design are therefore intentionally related but not
identical: `scripts/sign.py` provides the current signing core, while this
document defines the fuller supported signer that can grow around that core.
