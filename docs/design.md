...
| 3 | **Record forgery**, and **invisible-instruction smuggling** | Every character in Unicode categories Cc/Cf/Cs/Co is replaced with a space before serialisation — not just ASCII controls. Two Cf joiners (U+200C/U+200D) are held out because they carry no payload and spell real words. See §3.2 | multi-line text needs POST |
...
2. **Invisible characters survived `clean_text`.** Only C0 controls and `0x7F` were
   ...
   Held out by name (`SWEEP_EXEMPT`): U+200C and U+200D, the zero-width non-joiner and
   joiner. Both are Cf, but neither can encode a hidden instruction the way the tag block
   does, and both are orthographic in Brahmic scripts (ZWJ requests a conjunct, ZWNJ blocks
   one) and in Persian/Urdu. Sweeping them respells a correctly formed word for ~1B readers
   and, since a signature is verified against the swept text, makes a signed Indic write 403
   against text the caller never sent. Kept, a Brahmic conjunct and a ZWJ emoji sequence
   (👨‍👩‍👧) both round-trip. Every other invisible still goes.
