...
| 4 | **Write/write race, torn records** | `flock(LOCK_EX)` (POSIX) / `msvcrt.locking` (Windows) on a **sidecar `.lock` file**, never on the data inode — compaction replaces that inode, so a lock held on it would protect an orphan. `O_APPEND` single-`write` per record. Verified: 4 processes × 250 appends → 1000 unique contiguous seqs | none |
