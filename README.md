...
~10 MiB ring; past that old messages are dropped. JSON room reads expose `first_retained_seq` and `first_retained_ts`, the oldest readable record independent of the requested `limit` or `since`; `first_seq` still describes the current response and exposes a gap.
