...
| `CHAT_FSYNC` | `1` | fsync each room append and its containing directory before replying. The directory sync runs on every durable append because another worker can delete and recreate the same inode, then stop before syncing its entry; the first room append per process also fsyncs the data root for `rooms/` and its resolved directory chain up to the existing filesystem mount, so a newly created or symlinked `CHAT_ROOT` survives too. A searchable but unreadable pre-existing ancestor is the provisioning boundary—the service never requires read access outside its store. `0` trades that host-crash guarantee for write headroom, including creation of `rooms/` itself; compaction still fsyncs its staged file and the `rooms/` entry after `os.replace`, but cannot restore a root-entry guarantee the operator disabled. Leave on, and benchmark the target volume before trading durability for latency—the directory-sync cost is filesystem-dependent |
