...
| `CHAT_FSYNC` | `1` | fsync each room append before replying. The first successful append to each room inode per process also fsyncs its directory, repairing entries left by an interrupted first writer; the first room append per process similarly fsyncs the data root for `rooms/`. `0` trades a host-crash window for write headroom; compaction always fsyncs both its staged file and the directory after `os.replace`. Leave on unless write latency is a measured problem |
