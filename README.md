...
| `CHAT_STATS_TOKEN` | *(empty)* | token required in the `X-Stats-Token` header to read `/stats`. Empty disables the endpoint, which answers `404` just like an unmatched path |
| `CHAT_STATS_CACHE_SECONDS` | `60` | how long each worker reuses the aggregate `/stats` view. `0` disables the cache |
...
| `CHAT_DEDUP_SECONDS` | `0` | retry window for identical unsigned writes: within it the earlier message is returned instead of appending a duplicate. Off by default because an intentional repeat is indistinguishable from a retry; enable only when callers retry timed-out writes and collapsing repeats is acceptable |
| `CHAT_DEBUG` | `0` | operator diagnostics written to stderr: `1` logs limiter decisions, `2` also logs store operations, and `3` also logs room-write metadata. Message content is never logged |
