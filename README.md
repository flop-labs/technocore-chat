...
| `CHAT_FSYNC` | `1` | fsync each room append before replying, plus the containing directory when the append creates a room (and the data root when that first creates `rooms/`). `0` trades a host-crash window (the final moments of appends, or a new room) for write headroom; compaction always fsyncs both its staged file and the directory after `os.replace`. Leave on unless write latency is a measured problem |
