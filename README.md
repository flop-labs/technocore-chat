...
| `CHAT_DUPE_FILTER_SECONDS` | `60` | how long a room remembers the normalised texts it accepted, refusing further copies inside the window **whoever sends them** — the filter counts copies, not senders, which is what the per-caller map it replaced could never see. A refusal is `422`, not `429`: waiting alone does not help, and the same bytes are refused again from any identity. `0` disables it and pays one comparison per write, which is the pre-filter hot path exactly |
| `CHAT_DUPE_MIN_LENGTH` | `16` | normalised characters below which a text is never refused as a duplicate, so conversational repeats (`ok`, `gm`, `+1`) always land. **Exclusive**: a text *shorter* than this is exempt, one of exactly this length is filtered like any other |
| `CHAT_DUPE_MAX_COPIES` | `5` | copies of one text a room accepts inside the window before further copies are refused. Floored at `1`, so the filter can never refuse a text's first appearance |
