...
| `CHAT_EDGE_CACHE_SECONDS` | `1` | `s-maxage` plus `stale-while-revalidate` (`s-maxage * 5`) on `/rooms` and plain room reads so a CDN can collapse poll storms without serving more than a few seconds stale. Long-polls stay `no-store`; `0` disables both. Cloudflare needs a Cache Rule on these paths before it honors the header |
