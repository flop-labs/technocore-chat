...
| `CHAT_ROOMS_CACHE_SECONDS` | `3` | how long the `/rooms` directory walk is reused across callers. Structure is never stale — a room that was created, reaped or re-topiced is on the very next listing, from any worker — but activity recency (`idle_seconds`, `last_seq`, the ordering, the engagement aggregates) can lag by this long, because a message no longer invalidates the walk. `0` disables the cache and makes messages immediate too |
