...
def test_fsync_is_a_knob_but_compaction_never_skips_it(tmp_path, monkeypatch):
    ...
    # Each creation also persists its seq-state file and that file's entry in root.
    assert events == [
        "file",
        "bucket",
        "rooms",
    ...
    with config.override(FSYNC=False):
        ...
        assert len(calls) == 14  # the append skipped all syncs and the ancestor repair
        store.append(tmp_path, "p-fast-new", "bot", "fast creation")
        assert len(calls) == 14  # a new room skips directory and seq-state syncs too
        events.clear()
