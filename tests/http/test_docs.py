...
def test_openapi_limits_are_the_limits_the_server_enforces(client):
    ...
    # All three reading lanes clamp to MAX_LIMIT in the store, so all three publish it, pinned
    # to the one constant so they cannot drift. The bug was the silence, not the spelling: a
    # client asked for 250, was told of no ceiling, and got 200. It is published as prose rather
    # than as `maximum` because the server clamps instead of refusing, and a bound a validating
    # client trusts while the server ignores it is the one shape that misleads (docs/design.md
    # §3.5, #372/#402) — so require the ceiling to be stated and no unenforced bound beside it.
    # `/r/events` is in the list because it published no parameters at all, while the manual
    # sends readers there with since= and wait= "like any other room".
    for path in ("/r/{room}", "/rooms", "/r/events"):
        limit = next(p for p in doc["paths"][path]["get"]["parameters"] if p["name"] == "limit")
        assert f"1..{store.MAX_LIMIT}" in limit["description"], path
        assert "maximum" not in limit["schema"], path
